        ...
        {rows.map((a) => (
          ...
          >
            <div className="flex items-center gap-1.5">
              <span
                className={`rounded-full px-2 py-0.5 text-[15px] font-bold ${STATUS_TONE[a.status]}`}
              >
                {ARTICLE_STATUS_LABEL[a.status]}
              </span>
              <span className="text-[16px] text-muted">{a.category}</span>
              <span className="ml-auto text-[16px] text-muted">{a.date}</span>
            </div>
            <p className="mt-1.5 line-clamp-1 text-sm font-bold">{a.title}</p>
            <div className="mt-1.5 flex items-center gap-3 text-[16px] text-muted">
              <span>👁 {a.views}</span>
