    ...
    <div className="px-[18px] pb-10 pt-5">
        ...
        <div className="min-w-0">
          ...
          <div className="mt-1 flex flex-wrap items-center gap-1.5">
            ...
            {user.role === "reporter" && user.reporter_level && (
              <span className="rounded-full bg-line px-2 py-0.5 text-[16px] font-bold text-muted">
            ...
            {user.is_suspended && (
              <span className="rounded-full bg-muted px-2 py-0.5 text-[16px] font-bold text-white">
                정지됨
              </span>
            )}
          </div>
      ...
      {(user.businesses.length > 0 || myOrgs.length > 0) && (
        <section className="mt-4">
          <h2 className="mb-2 text-[18px] font-bold text-muted">내 소속</h2>
          <ul className="flex flex-col gap-2">
            {user.businesses.map((b) => (
              <li key={b.id}>
                ...
                >
                  <span className="rounded-full bg-tag-biz-bg px-2.5 py-1 text-[16px] font-bold text-tag-biz-fg">
                    🏪 업체
                  </span>
                  ...
                  {b.status === "approved" ? (
                    <span className="text-[16px] text-muted">보기 ›</span>
                  ) : (
                    <span className="text-[16px] text-muted">승인 대기</span>
                  )}
                </Link>
              </li>
            ))}
