...
export function generateMetadata({ params }: { params: { handle: string } }) {
  ...
  return {
    ...
    // openGraph를 정의하면 상위 것을 대체하므로 이미지·siteName·locale을
    // 여기에도 넣는다.
    openGraph: {
      title,
      description: s.bio,
...
export default async function ReporterProfilePage({
  ...
  return (
    <div className="px-[18px] py-5">
      ...
      <h1 className="mt-2 text-[27px] font-extrabold">{staff.name}</h1>
      ...
      <section className="mt-7">
        ...
        {mine.length === 0 ? (
          <p className="mt-2 rounded-card border border-line bg-white p-5 text-center text-[18px] text-muted">
            아직 등록된 기사가 없습니다
          </p>
        ) : (
          <ul>
