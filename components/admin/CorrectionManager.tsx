      ...
      <ul className="flex flex-col gap-2.5">
        {rows.map((r) => (
          ...
          >
            ...
            {r.reason && (
              <p className="mt-1 text-[17px] text-muted">사유: {r.reason}</p>
            )}
            <p className="mt-1 whitespace-pre-line text-[18px] leading-relaxed">
              {r.body}
            </p>
            <p className="mt-1 text-[16px] text-muted">{r.createdAt}</p>
            {r.status === "pending" && (
