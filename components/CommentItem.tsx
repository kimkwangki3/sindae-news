  ...
  return (
    <li className="border-t border-line pt-4 first:border-t-0">
      <div className="flex items-center justify-between">
        <span className="text-[18px] font-bold">{comment.author}</span>
        <div className="flex items-center gap-2">
          <span className="text-[16px] text-muted">{comment.createdAt}</span>
          {reportNode}
        </div>
      </div>
      ...
      {comment.mine && !isTmp && !editing && (
        <div className="mt-1.5 flex gap-3">
          <button
            ...
            className="text-[16px] text-muted underline"
          >
            수정
          </button>
          <button
            ...
            onClick={remove}
            disabled={pending}
            className="text-[16px] text-muted underline disabled:opacity-50"
          >
            삭제
          </button>
        </div>
