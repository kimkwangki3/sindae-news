  ...
  return (
    <section className="mx-[18px] mt-4 rounded-card border border-line bg-ivory-2 p-4">
      <div className="flex items-baseline justify-between">
        <h2 className="text-[14px] font-bold text-rose-deep">📌 생활정보</h2>
      ...
      <ul className="mt-3 flex flex-wrap gap-2">
        {pages.map((p) => (
          <li key={p.slug}>
            <Link
              href={`/info/${p.slug}`}
              className="flex min-h-[44px] items-center gap-1.5 rounded-element border border-line bg-white px-3 text-[13px] text-ink"
            >
