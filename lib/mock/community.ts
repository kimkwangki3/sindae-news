...
const MARKET_COLS =
  "id, category, title, neighborhood, body, is_pinned, created_at, author:profiles(nickname), market_comments(count), market_photos(count)";
...
export async function getMarketPost(id: string): Promise<MarketPost | null> {
  ...
  const { data } = await supabase
    ...
    .eq("visibility", "visible")
    .maybeSingle();
  return data ? toMarketPost(data as Record<string, unknown>) : null;
}
...
const BOARD_COLS =
  "id, category, title, body, like_count, view_count, is_pinned, created_at, author:profiles(nickname), board_comments(count)";
